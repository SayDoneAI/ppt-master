...
describe('design_patch', () => {
  ...
  it('围绕当前选中元素生成 AI 指令请求', () => {
    ...
    expect(promptDownload.content).toContain('Design Editor Local AI Handoff')
    ...
    expect(promptDownload.content).toContain('项目主产物 / 主预览仍以 SVG 页面为准')
    expect(promptDownload.content).toContain('compat state')
    expect(promptDownload.content).not.toContain('唯一真相源')
